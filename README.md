...
![Lanterna](assets/teaserimage.png)
...
Built on [Meta Wearables DAT SDK](https://github.com/facebook/meta-wearables-dat-ios) (iOS) + [Gemini Live API](https://ai.google.dev/gemini-api/docs/live) + [OpenClaw](https://github.com/nichochar/openclaw) (optional).

Lanterna is a fork of [VisionClaw](https://github.com/sseanliu/VisionClaw), focused on iOS and on making the assistant genuinely usable for blind and low-vision users.

**Supported platforms:** iOS (iPhone)
...
```bash
git clone https://github.com/fmaule/Lanterna.git
cd Lanterna
open Lanterna.xcodeproj
```
...
```bash
cp Lanterna/Secrets.swift.example Lanterna/Secrets.swift
```
...
All source code is in `Lanterna/`:
...
**Key details:**
- **Signaling server**: Node.js + WebSocket, located at `server/` -- serves the browser viewer and relays SDP/ICE
...
For full details, see [`Lanterna/WebRTC/README.md`](Lanterna/WebRTC/README.md).
...
Lanterna is a fork of VisionClaw. If you use it in your research, please cite the original VisionClaw paper:
